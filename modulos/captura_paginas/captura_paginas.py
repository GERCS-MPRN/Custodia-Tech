...
def rolagem_captura(
    driver, caminho, nome_arquivo, ponto_interrupcao, rolagem_interrompida
):
    ...
    height = driver.execute_script(
        "return Math.min(" "arguments[0].clientHeight, arguments[0].offsetHeight);",
        element,
    )
    ...
    extra_height = offset - driver.execute_script(
    ...
    for aba in abas:
        ...
        while True:
            ...
            if stop == True:
                ...
                driver.execute_script(
                    "window.alert('A rolagem com captura automática foi interrompida para essa página! Após selecionar o ponto de captura (fundo da página visível), pressione ctrl+alt+F6 para continuar.');"
                )
                break

            offset += height
            driver.execute_script(
                "arguments[0].scrollTo(0, arguments[1]);", element, offset
            )
        ...
        ponto_interrupcao = (
            driver.execute_script("return arguments[0].scrollTop;", element) + height
        )

        dict_captura.append(
            {
                "url": driver.current_url,
                "rolagem_interrompida": rolagem_interrompida,
                "ponto_interrupcao": ponto_interrupcao,
            }
        )
...
def completa_barra_progresso(driver):
    ...
    driver.execute_script(
        "window.alert('Processamento concluído! Página(s) capturada(s) com sucesso!');"
    )
...
def captura_paginas(
    ...
    for aba in abas:
        ...
        caminho = (
            destino_captura + "\\" + str(conta_capturas).zfill(3) + " - " + nome_arquivo
        )
        ...
        visible_html_array = rolagem_captura(
            driver, caminho, nome_arquivo, ponto_interrupcao, rolagem_interrompida
        )
        ...
        def get_media_info(url):
            try:
                ...
                response = requests.get(
                    url, headers=headers, allow_redirects=True, timeout=5, verify=False
                )
                ...
                if response.status_code in [200, 206] and any(
                    ct in content_type for ct in mime_to_extension
                ):
        ...
        def download_media(session, media_url):
            try:
                ...
                response = session.get(
                    media_url, headers=headers, verify=False, stream=True, timeout=5
                )
        ...
        media_info = [
            info
            for info in (get_media_info(url) for url in lista_midias)
            if info is not None
        ]

        with ThreadPoolExecutor(max_workers=4) as executor:
            future_to_info = {
                executor.submit(download_media, session, media_url): (
                    media_url,
                    extension,
                )
                for media_url, extension in media_info
            }
                    ...
                    try:
                        ...
                        if media_data:
                            ...
                            safe_file_name = "".join(
                                ...
                                for c in final_file_name
                            )  # Evita caracteres inválidos

                            # Salva a mídia em um arquivo.
                            with open(
                                os.path.join(midias_dir, safe_file_name), "wb"
                            ) as handler:
                            ...
                            print(
                                f"Falha ao baixar {url}. Tentando novamente em 10 segundos..."
                            )
                            time.sleep(
                                10
                            )  # Excepcionalmente, aguarda 10 segundos antes de tentar novamente
                            tentativa += 1
                    except:
                        print(
                            f"Falha ao baixar {url}. Tentando novamente em 10 segundos..."
        ...
        converte_para_pdf(
            caminho + "\\" + nome_arquivo + ".png",
            caminho + "\\" + nome_arquivo + ".pdf",
        )
        ...
        if modo_captura == 1:
            ...
            driver_monitoramento.execute_script(
...
def iniciar_captura_de_paginas(case_directory, urls):
    ...
    stop_thread_continuacao = False
    stop = False
    thread_atalho_interrupcao = threading.Thread(
        target=aguarda_atalho_interrupcao, daemon=True
    )
    thread_atalho_interrupcao.start()
    thread_atalho_continuacao = threading.Thread(
        target=aguarda_atalho_continuacao, daemon=True
    )
    ...
    try:
        ...
        if len(urls) > 0:
            ...
            for url in urls:
                ...
                # Espera que a página esteja completamente carregada (HTML)
                while (
                    d := driver.execute_script("return document.readyState")
                ) != "complete":
                    time.sleep(0.5)
        ...
        while True:
            ...
            if len(driver.window_handles) == 0:
                ...
                driver.execute_script(
                    f"""window.alertResult = confirm("Deseja capturar as páginas no modo abaixo?\\n\\nMODO DE CAPTURA: {modo_ajuste}.");"""
                )
        ...
        if modo_captura == 0:
            ...
            captura_paginas(
                driver, None, destino_captura, zip_files, config_captura, modo_captura
            )
            ...
            for target in targets["targetInfos"]:
                if target["type"] == "page" and target["url"].startswith("http"):
                    ...
                    cookies = driver.execute_cdp_cmd("Storage.getCookies", {})[
                        "cookies"
                    ]
                    ...
                    driver.execute_cdp_cmd(
                        "Target.detachFromTarget", {"sessionId": session["sessionId"]}
                    )
            ...
            options_monitoramento.add_experimental_option(
                "excludeSwitches", ["enable-automation"]
            )
            options_monitoramento.add_experimental_option(
                "useAutomationExtension", False
            )
            options_monitoramento.add_argument(
            ...
            for url, cookies in cookies_by_url.items():
                ...
                # Espera que a página esteja completamente carregada (HTML)
                while (
                    d := driver_headless.execute_script("return document.readyState")
                ) != "complete":
                    time.sleep(0.5)
            ...
            if ajuste_pagina == 0:
                config_captura = rolagem_automatica_paginas(
                    driver_headless, ajuste_pagina
                )
        ...
        print(
            "O navegador foi fechado de modo manual pelo usuário ou ocorreu um erro interno na aplicação."
        )
