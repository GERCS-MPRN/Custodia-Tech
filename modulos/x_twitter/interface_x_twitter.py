...
def janela_x_twitter(tk_custodia_tech, case_directory, file_data_x, usuario, icone_custodia):
    def download_x_video():
        ...
        if url.strip():  # Verifica apenas URLs não vazios
            ...
            if not (url_limpo.startswith('twitter.com/') or url_limpo.startswith('x.com/')):
    ...
    LARGURA_CT, ALTURA_CT = centraliza_janela_no_monitor_ativo(tk_x_twittter, "x_twitter")

    label_link = tk.Label(tk_x_twittter, text="URL do vídeo do X (Twitter):")
    label_link.pack(pady=int(ALTURA_CT * 0.0156))

    entry_url = ttk.Entry(tk_x_twittter, bootstyle="dark", width=int(LARGURA_CT * 0.125))
