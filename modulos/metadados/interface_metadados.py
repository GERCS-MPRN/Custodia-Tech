...
def janela_extrair_metadados(tk_custodia_tech, case_directory, usuario, icone_custodia, metadados_total):
    ...
    def extrair_e_fechar():
        if any(var.get() for _, var in checkboxes):
            ...
            tk.messagebox.showwarning(
                "Aviso", "Por favor, selecione pelo menos um arquivo antes de extrair os metadados.", parent=tk_extrair_metadados)
    ...
    pasta = filedialog.askdirectory(parent=tk_custodia_tech, title="Selecione uma pasta...")
    if not pasta:
        return
    ...
    LARGURA_CT, ALTURA_CT = centraliza_janela_no_monitor_ativo(tk_extrair_metadados, "extrair_metadados")
    ...
    titulo_label = tk.Label(tk_extrair_metadados, text=titulo_texto, font=("Arial", tamanho_fonte, "bold"))
    ...
    selecionar_todos_checkbox = ttk.Checkbutton(
        files_frame, text="Selecionar Todos", variable=selecionar_todos_var, command=selecionar_todos)
    selecionar_todos_checkbox.grid(row=0, column=0, padx=5, pady=5, sticky="w")

    # Listar arquivos e criar checkboxes
    for i, arquivo in enumerate(os.listdir(pasta)):
        caminho_completo = os.path.join(pasta, arquivo)
        if os.path.isfile(caminho_completo):
            var = tk.BooleanVar()
            checkbox = ttk.Checkbutton(files_frame, text=arquivo, variable=var)
    ...
    progress_bar = ttk.Progressbar(tk_extrair_metadados, orient="horizontal", length=300, mode="determinate")
    ...
    extrair_button = ttk.Button(
        tk_extrair_metadados, text="Extrair Metadados", command=extrair_e_fechar, bootstyle="info", style='TButton')
