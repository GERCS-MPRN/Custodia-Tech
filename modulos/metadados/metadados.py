...
class CustomJSONEncoder(json.JSONEncoder):
    def default(self, obj):
        if isinstance(obj, (Fraction, int, float)):  # Converte IFDRational e números para float
            return float(obj)
        elif isinstance(obj, bytes):  # Converte bytes para string legível
            return obj.decode(errors='ignore')
        elif isinstance(obj, datetime.datetime):  # Converte datetime para string formatada
            return obj.isoformat()
        elif isinstance(obj, tuple):  # Converte tuplas para listas
            return list(obj)
        elif isinstance(obj, set):  # Converte conjuntos para listas
            return list(obj)
        elif isinstance(obj, dict):  # Converte dicionários com chaves não string para strings
            return {str(k): v for k, v in obj.items()}
        elif hasattr(obj, "__dict__"):  # Converte objetos personalizados para dicionários
            return obj.__dict__
    ...
    if isinstance(exif_data, dict):
        ...
        for key, value in exif_data.items():
            if isinstance(value, (Fraction, int, float, str, list, dict, type(None))):
                normalized_data[key] = float(value) if isinstance(value, Fraction) else value  # Converte Fraction
            elif isinstance(value, tuple):
                normalized_data[key] = list(value)  # Converte tuple para list
            elif isinstance(value, bytes):
                normalized_data[key] = value.decode(errors='ignore')  # Converte bytes para string
            else:
                normalized_data[key] = str(value)  # Converte qualquer outro tipo para string
...
def get_exiftool_metadata(file_path):
    ...
    with exiftool.ExifTool(EXIFTOOL_PATH) as et:
        ...
        if metadata:
            ...
            author_fields = [
                "Author", "XPAuthor", "OwnerName", "Creator", "FileOwner",
                "Copyright", "Artist"
            ]
...
def captar_metadados(file_path):
    ...
    metadata = {
        ...
        "Created": datetime.datetime.fromtimestamp(stat_info.st_ctime).strftime('%d/%m/%Y %H:%M:%S'),
        "Modified": datetime.datetime.fromtimestamp(stat_info.st_mtime).strftime('%d/%m/%Y %H:%M:%S'),
        "Accessed": datetime.datetime.fromtimestamp(stat_info.st_atime).strftime('%d/%m/%Y %H:%M:%S'),
...
def extracao_metadados(arquivo, case_directory, usuario, metadados_total):
    ...
    try:
        json_metadata = json.dumps(metadata, indent=4, ensure_ascii=False, cls=CustomJSONEncoder)
        print(json_metadata)
        
        output_file = os.path.join(case_directory, f"{os.path.basename(arquivo)}.metadata.json")
    ...
    metadados_total.append({
        'nome_do_arquivo': os.path.basename(novo_zip_path),
        'hashes': hashes,
        'metadata': metadata
    })
