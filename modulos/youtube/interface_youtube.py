...
def janela_youtube(tk_custodia_tech, case_directory, usuario, file_data_youtube, icone_custodia):
        ...
        while len(comments) < max_comments:
            try:
                ...
                for item in response['items']:
                    ...
                    author_name = snippet.get('authorDisplayName', 'Autor não disponível')
                    author_channel_id = snippet.get('authorChannelId', {}).get('value', 'ID do canal não disponível')
                    comment_text = snippet.get('textDisplay', 'Comentário não disponível')
                    comments.append(f"Autor: {author_name} (Channel ID: {author_channel_id})\nComentário: {comment_text}\n")
    ...
    def download_youtube_videos():
        ...
        for url in urls:
            if url.strip():  # Verifica apenas URLs não vazios
                ...
                if not (url_limpo.startswith('youtube.com/watch?v=') or 
                        url_limpo.startswith('youtu.be/')):
        ...
        for i, urlvideo in enumerate(urls, start=1):
            ...
            try:
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    ...
                    comments_text = '\n'.join(comments) if comments else 'Comentários não disponíveis'
                    ...
                    file_data_youtube.append({'nome_do_arquivo': file_name, 'hashes': hashes, 'metadata': metadata, 'urlvideo': urlvideo})
                ...
                shutil.rmtree(video_directory)
                messagebox.showerror("Erro", f"Erro ao baixar o vídeo {i}: Verifique sua conexão com a Internet ou o link inserido.", parent=tk_youtube)
                button.config(state=tk.NORMAL)

        # Exibe a mensagem de conclusão após todos os downloads
        if completed_downloads > 0:
            messagebox.showinfo("Concluído", f"Download de {completed_downloads} vídeo(s) concluído(s) com sucesso!", parent=tk_youtube)
        else:
            messagebox.showinfo("Concluído", "Nenhum vídeo foi baixado.", parent=tk_youtube)
    ...
    label_link = tk.Label(tk_youtube, text="URLs dos vídeos do YouTube (máximo 5):", font=fonte)
    ...
    progress_b = ttk.Progressbar(tk_youtube, orient="horizontal",
    ...
    button = ttk.Button(tk_youtube, text="Baixar Vídeo(s)", 
