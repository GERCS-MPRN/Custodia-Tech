...
def janela_whatsapp(
    ...
    def captura_whatsapp(file_data_whatsapp):
        ...
        thread_captura_whatsapp = Thread(
            target=captura_thread, args=(selecao, file_data_whatsapp)
        )
    ...
    def captura_thread(selecao, file_data_whatsapp):
        try:
            ...
            if selecao == 1:
                messagebox.showinfo(
                    "Captura de Lista de Contatos de Whatsapp\n\n",
                ...
                file_data_whatsapp = metadados_whatsapp(
                    case_directory, file_data_whatsapp
                )

            if selecao == 2:
                messagebox.showinfo(
                    "Captura Conversas de Whatsapp\n\n",
                    "1. Efetue o login no Whatsapp com o QRCode.\n"
    ...
    style.configure(
        ...
        foreground="white",
        background="green",
        font=("Helvetica", tamanho_fonte_padrao),
    )

    # Cria label de instruções
    label_instrucoes = tk.Label(
        tk_whatsapp,
        text="Selecione o tipo de captura",
        justify="center",
        font=fonte_titulo,
    )
    ...
    radio_contatos = tk.Radiobutton(
        tk_whatsapp,
        text="Capturar Lista de Contatos",
        variable=var,
        value=1,
        font=fonte_padrao,
    )
    ...
    radio_conversas = tk.Radiobutton(
        tk_whatsapp, text="Capturar Conversas", variable=var, value=2, font=fonte_padrao
    )
    ...
    iniciar_button = ttk.Button(
        tk_whatsapp,
        text="Iniciar",
        command=lambda: captura_whatsapp(file_data_whatsapp),
