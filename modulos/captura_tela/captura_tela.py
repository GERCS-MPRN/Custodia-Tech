...
def select_area(tk_custodia_tech):
    ...
    canvas = tk.Canvas(overlay, cursor="cross", bg="gray", width=total_width, height=total_height)
...
def capture_screenshot(case_directory, usuario, tk_custodia_tech, tk_captura_tela, bounding_box=None):
    """
...
def capture_and_process_screenshot(case_directory, algorithms, reason, usuario, tk_custodia_tech, captured_screenshots, captured_screenshots_zip, tk_captura_tela, bounding_box):
    """
    Captura e processa uma screenshot, incluindo compactação e cálculo de hashes
    ...
    if reason and reason[0].islower():
        reason = reason[0].upper() + reason[1:]  # Converte o primeiro caractere para maiúsculo
        
    screenshot_path = capture_screenshot(case_directory, usuario, tk_custodia_tech, tk_captura_tela, bounding_box)
    if screenshot_path:
        captured_screenshots.append((screenshot_path))
        # Compacta a imagem em um .zip
        ...
        hash_results = calculate_hash(zip_file_path, algorithms)
        file_metadata = get_file_metadata(zip_file_path)
        
        captured_screenshots_zip.append((hash_results, file_metadata, reason))
        
        return hash_results, file_metadata, reason
    return None
